...
public:
    ...
    static float Float() {
        return static_cast<float>(s_Distribution(s_RandomEngine)) / static_cast<float>(
...
void ParticleSystem::OnRender(const GE::OrthographicCamera &camera) {
    ...
    for (auto &particle : m_ParticlePool) {
        ...
        GE::Renderer2D::DrawRotatedQuad({particle.Position.x, particle.Position.y, 0.3f},
